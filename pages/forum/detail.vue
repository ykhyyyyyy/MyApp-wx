<template>
  <view class="container">
    <custom-nav-bar title="帖子详情" :show-back="true"></custom-nav-bar>
    <view v-if="isLoading" class="loading"><uni-load-more status="loading" content-text="加载中..."></uni-load-more></view>
    <view v-else-if="!post" class="empty">未找到帖子</view>
    <view v-else class="detail-box">
      <view class="post-title">{{post.title}}</view>
      <view class="post-content">{{post.content}}</view>
      <view class="post-info">作者：{{post.username}} | 浏览：{{post.views}}</view>
      <view class="actions">
        <button class="del-btn" @click="deletePost">删除</button>
      </view>
...
export default {
  ...
  computed: {
    ...mapState('post', ['currentPost','postComments','isLoading']),
    post() { return this.currentPost; },
    comments() { return this.postComments; }
  },
  methods: {
    ...mapActions('post', ['getPostDetail','getCommentPage','createComment','deletePost','deleteComment']),
    async loadDetail() {
      await this.getPostDetail(this.id);
      await this.getCommentPage({postId:this.id});
    },
    async addComment() {
      if (!this.input.trim()) return;
      await this.createComment({postId:this.id,userId:this.userId,content:this.input});
      this.input='';
      this.getCommentPage({postId:this.id});
    },
    async deletePost() {
      uni.showModal({title:'确认删除',content:'确定要删除该帖子吗？',success:async(res)=>{
        if(res.confirm){await this.deletePost(this.id);uni.showToast({title:'删除成功',icon:'success'});setTimeout(()=>uni.navigateBack(),1000);}
      }});
    },
    async deleteComment(id) {
      uni.showModal({title:'确认删除',content:'确定要删除该评论吗？',success:async(res)=>{
        if(res.confirm){await this.deleteComment(id);this.getCommentPage({postId:this.id});}
      }});
    }
  },
...
.post-title{font-size:36rpx;font-weight:bold;color:#333;margin-bottom:20rpx;}
.post-content{font-size:30rpx;color:#666;margin-bottom:20rpx;}
.post-info{font-size:24rpx;color:#999;margin-bottom:20rpx;}
